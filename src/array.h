...
static inline void array_from_array_sexp(SEXP array_sexp, struct ArchArray* array, const char* arg) {
  if (!Rf_inherits(array_sexp, "arch_array")) {
    Rf_error("`x` must be an `arch_array()`");
  }
  ...
  if (arch_array_set_schema(array, schema, &status) != 0) {
    Rf_error("%s [%s]", status.message, strerror(status.code));
  }

  if (arch_array_set_array(array, array_data, &status) != 0) {
    Rf_error("%s [%s]", status.message, strerror(status.code));
  }
}
