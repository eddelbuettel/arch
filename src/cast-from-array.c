...
#define LOOP_NODATA(_body) \
  unsigned char* validity_buffer = arch_array_validity_buffer(&array); \
...
SEXP arch_c_logical_from_array(SEXP array_sexp) {
  ...
  int copy_result = arch_buffer_copy_value(
    result, arch_TYPE_INT32,
    arch_array_data_buffer(&array), array.type,
    size, offset
  );
...
SEXP arch_c_integer_from_array(SEXP array_sexp) {
  struct ArchArray array;
  array_from_array_sexp(array_sexp, &array, "x");
  int64_t size = array.array_data->length;
  int64_t offset = array.array_data->offset;
...
SEXP arch_c_double_from_array(SEXP array_sexp) {
  struct ArchArray array;
  array_from_array_sexp(array_sexp, &array, "x");
  int64_t size = array.array_data->length;
  int64_t offset = array.array_data->offset;
...
SEXP arch_c_raw_from_array(SEXP array_sexp) {
  struct ArchArray array;
  array_from_array_sexp(array_sexp, &array, "x");
  int64_t size = array.array_data->length;
  int64_t offset = array.array_data->offset;
...
SEXP arch_c_character_from_array(SEXP array_sexp) {
  struct ArchArray array;
  array_from_array_sexp(array_sexp, &array, "x");
  int64_t size = array.array_data->length;
  int64_t offset = array.array_data->offset;
  ...
  void* data_buffer = arch_array_data_buffer(&array);
  if (data_buffer == NULL) {
    Rf_error(
  ...
  switch (array.type) {
  case arch_TYPE_STRING:
  case arch_TYPE_BINARY: {
    const int32_t* offsets = arch_array_offset_buffer(&array);
    for (int64_t i = 0; i < size; i++) {
  ...
  case arch_TYPE_LARGE_STRING:
  case arch_TYPE_LARGE_BINARY: {
    const int64_t* large_offsets = arch_array_large_offset_buffer(&array);
    for (int64_t i = 0; i < size; i++) {
  ...
  case arch_TYPE_FIXED_SIZE_BINARY: {
    for (int64_t i = 0; i < size; i++) {
      SET_STRING_ELT(
        result_sexp,
        i,
  ...
  case arch_TYPE_INT8:
  case arch_TYPE_UINT8:
  case arch_TYPE_INT16:
  case arch_TYPE_UINT16:
  case arch_TYPE_INT32:
  case arch_TYPE_UINT32:
  case arch_TYPE_INT64: // this may be lossy for int64
  case arch_TYPE_UINT64: // this may be lossy for uint64
  case arch_TYPE_HALF_FLOAT:
  case arch_TYPE_FLOAT:
  case arch_TYPE_DOUBLE: {
    int double_copy_result;
    double double_val;
    char double_buffer[32];
    memset(double_buffer, 0, 32);
    for (int64_t i = 0; i < size; i++) {
      double_copy_result = arch_buffer_copy_value(
        &double_val, arch_TYPE_DOUBLE,
        data_buffer, array.type,
        1, offset + i
      );

      if (double_copy_result != 0) {
        Rf_error("arch_buffer_copy_value() failed with code %d", double_copy_result);
      }
