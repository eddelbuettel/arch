...
int arch_array_copy(struct ArchArray* array_dst, int64_t dst_offset,
  ...
  if (array_dst == NULL) {
    arch_status_set_error(status, EINVAL, "`array_dst` is NULL");
    RETURN_IF_NOT_OK(status);
  }

  if (array_src == NULL) {
    arch_status_set_error(status, EINVAL, "`array_src` is NULL");
    RETURN_IF_NOT_OK(status);
  }

  // dense unions aren't supported yet
  if (array_src->type == arch_TYPE_DENSE_UNION) {
    arch_status_set_error(status, EINVAL, "Dense unions are not supported by arch_array_copy()");
    RETURN_IF_NOT_OK(status);
  }
  ...
  unsigned char* validity_buffer_src = arch_array_validity_buffer(array_src);
  unsigned char* validity_buffer_dst = arch_array_validity_buffer(array_dst);

  int32_t* offset_buffer_src = arch_array_offset_buffer(array_src);
  int32_t* offset_buffer_dst = arch_array_offset_buffer(array_dst);

  int64_t* large_offset_buffer_src = arch_array_large_offset_buffer(array_src);
  int64_t* large_offset_buffer_dst = arch_array_large_offset_buffer(array_dst);

  char* union_type_buffer_src = arch_array_union_type_buffer(array_src);
  char* union_type_buffer_dst = arch_array_union_type_buffer(array_dst);

  void* data_buffer_src = arch_array_data_buffer(array_src);
  void* data_buffer_dst = arch_array_data_buffer(array_dst);

  if (which_buffers & arch_BUFFER_VALIDITY) {
    // note that the validity buffer can be NULL if the null count is 0
    if (validity_buffer_src != NULL) {
      if (validity_buffer_dst == NULL) {
        arch_status_set_error(status, EINVAL, "Can't copy validity buffer to NULL");
        RETURN_IF_NOT_OK(status);
      }
  ...
  if (which_buffers & arch_BUFFER_OFFSET) {
    if (offset_buffer_src != NULL) {
      if (offset_buffer_dst == NULL) {
        arch_status_set_error(status, EINVAL, "Can't copy offset buffer to NULL");
        RETURN_IF_NOT_OK(status);
      }
    ...
    if (large_offset_buffer_src != NULL) {
      if (large_offset_buffer_dst == NULL) {
        arch_status_set_error(status, EINVAL, "Can't copy large offset buffer to NULL");
        RETURN_IF_NOT_OK(status);
      }
  ...
  if (which_buffers & arch_BUFFER_UNION_TYPE) {
    if (union_type_buffer_dst != NULL) {
      if (large_offset_buffer_dst == NULL) {
        arch_status_set_error(status, EINVAL, "Can't copy union type buffer to NULL");
        RETURN_IF_NOT_OK(status);
      }
  ...
  } else if (array_src->type == arch_TYPE_FIXED_SIZE_LIST) {
  ...
  if (which_buffers & arch_BUFFER_DATA) {
    if (data_buffer_src != NULL) {
      if (array_src->element_size_bytes > 0) {
        if (data_buffer_dst == NULL) {
          arch_status_set_error(status, EINVAL, "Can't copy data buffer to NULL");
          RETURN_IF_NOT_OK(status);
        }
  ...
  struct ArchArray child_array_src;
  struct ArchArray child_array_dst;

  if (which_buffers & arch_BUFFER_CHILD) {
    // copy child arrays
    for (int64_t i = 0; i < array_src->schema->n_children; i++) {
      arch_array_init(&child_array_src, array_src->schema->children[i], array_src->array_data->children[i], status);
      RETURN_IF_NOT_OK(status);

      arch_array_init(&child_array_dst, array_dst->schema->children[i], array_dst->array_data->children[i], status);
      RETURN_IF_NOT_OK(status);
  ...
  if (which_buffers & arch_BUFFER_DICTIONARY) {
    // copy dictionary array (the whole dictionary array)
    if (array_src->schema->dictionary != NULL) {
      arch_array_init(&child_array_src, array_src->schema->dictionary, array_src->array_data->dictionary, status);
      RETURN_IF_NOT_OK(status);

      arch_array_init(&child_array_dst, array_src->schema->dictionary, array_dst->array_data->dictionary, status);
      RETURN_IF_NOT_OK(status);

      arch_array_copy(&child_array_dst, 0, &child_array_src, 0, child_array_src.array_data->length, which_buffers, status);
      RETURN_IF_NOT_OK(status);
    }
  }
