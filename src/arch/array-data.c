...
void arch_array_release_internal(struct ArrowArray* array_data) {
  ...
  if (array_data->children != NULL) {
    for (uint64_t i = 0; i < array_data->n_children; i++) {
      arch_array_release_internal(array_data->children[i]);
  ...
  if (array_data->dictionary != NULL) {
    arch_array_release_internal(array_data->dictionary);
...
int arch_array_copy_structure(struct ArrowArray* out, struct ArrowArray* array_data,
  ...
  out->release = &arch_array_release_internal;
  ...
  if (array_data->n_children > 0) {
    ...
    if (which_buffers & arch_BUFFER_CHILD) {
      for (uint64_t i = 0; i < array_data->n_children; i++) {
        ...
        result = arch_array_copy_structure(out->children[i], array_data->children[i], which_buffers);
        if (result != 0) {
          out->release(out);
          return result;
        }
  ...
  if (which_buffers & arch_BUFFER_DICTIONARY) {
    if (array_data->dictionary != NULL) {
      ...
      result = arch_array_copy_structure(out->dictionary, array_data->dictionary, which_buffers);
      if (result != 0) {
        out->release(out);
        return result;
      }
