...
int arch_array_alloc_buffers(struct ArchArray* array, int32_t which_buffers, struct ArrowStatus* status) {
  ...
  int has_existing_offset_buffer = arch_array_offset_buffer(array) != NULL;
  int has_existing_large_offset_buffer = arch_array_large_offset_buffer(array) != NULL;
  ...
  if (which_buffers & arch_BUFFER_OFFSET) {
    int32_t* offset_buffer = arch_array_offset_buffer(array);
    ...
    int64_t* large_offset_buffer = arch_array_large_offset_buffer(array);
  ...
  if (which_buffers & arch_BUFFER_UNION_TYPE) {
    char* union_type_buffer = arch_array_union_type_buffer(array);
  ...
  struct ArchArray child_array;

  if (which_buffers & arch_BUFFER_CHILD) {
    // also allocate child arrays
    for (int64_t i = 0; i < array->schema->n_children; i++) {
      arch_array_init(&child_array, array->schema->children[i], array->array_data->children[i], status);
      RETURN_IF_NOT_OK(status);

      arch_array_alloc_buffers(&child_array, which_buffers, status);
      RETURN_IF_NOT_OK(status);
    }
  }
  ...
  if (which_buffers & arch_BUFFER_DICTIONARY) {
    ...
    if (array->schema->dictionary != NULL) {
      arch_array_init(&child_array, array->schema->dictionary, array->array_data->dictionary, status);
      RETURN_IF_NOT_OK(status);
