...
void arch_schema_release_internal(struct ArrowSchema* schema) {
  ...
  if (schema->children != NULL) {
    for (uint64_t i = 0; i < schema->n_children; i++) {
      arch_schema_release_internal(schema->children[i]);
  ...
  if (schema->dictionary != NULL) {
    arch_schema_release_internal(schema->dictionary);
...
int64_t arch_schema_metadata_size(const char* metadata) {
  if (metadata == NULL) {
    return 0;
  }
...
void arch_schema_reset_internal(struct ArrowSchema* schema) {
  schema->format = NULL;
  schema->name = NULL;
  ...
  schema->release = &arch_schema_release_internal;
}
...
int arch_schema_deep_copy(struct ArrowSchema* out, struct ArrowSchema* schema) {
  ...
  arch_schema_reset_internal(out);
  ...
  int64_t metadata_size = arch_schema_metadata_size(schema->metadata);
  if (metadata_size > 0) {
  ...
  if (out->n_children > 0) {
    ...
    for (int64_t i = 0; i < out->n_children; i++) {
      ...
      result = arch_schema_deep_copy(out->children[i], schema->children[i]);
      if (result != 0) {
        out->release(out);
        return result;
      }
  ...
  if (schema->dictionary != NULL) {
    ...
    result = arch_schema_deep_copy(out->dictionary, schema->dictionary);
    if (result != 0) {
      out->release(out);
      return result;
    }
