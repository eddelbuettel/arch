...
SEXP arch_c_array_from_sexp(SEXP buffers_sexp, SEXP length_sexp, SEXP null_count_sexp,
  ...
  Rf_setAttrib(array_prot, R_ClassSymbol, Rf_mkString("arch_array_data_prot"));
...
SEXP arch_c_array_info(SEXP array_data_xptr) {
  ...
  if (array_prot != R_NilValue && Rf_inherits(array_prot, "arch_array_data_prot")) {
...
// for ArrowArray* that were created by arch_c_array_from_sexp()
