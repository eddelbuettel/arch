...
SEXP arch_c_schema_xptr_new(SEXP format_sexp, SEXP name_sexp, SEXP metadata_sexp,
  ...
  Rf_setAttrib(schema_prot, R_ClassSymbol, Rf_mkString("arch_schema_prot"));
...
SEXP arch_c_schema_deep_copy(SEXP schema_xptr) {
  ...
  int result = arch_schema_deep_copy(new_schema, schema);
  if (result != 0) {
    Rf_error("arch_schema_copy() failed with code %d (%s)", result, strerror(result));
  }
...
SEXP arch_c_schema_data(SEXP schema_xptr) {
  struct ArrowSchema* schema = schema_from_xptr(schema_xptr, "schema");
  ...
  if (Rf_inherits(schema_prot, "arch_schema_prot")) {
...
// for ArrowSchema* that were created by arch_c_schema_xptr_new()
