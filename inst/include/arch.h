...
static inline struct ArrowSchema* schema_from_xptr(SEXP schema_xptr, const char* arg) {
  if (!Rf_inherits(schema_xptr, "arch_schema")) {
    Rf_error("`%s` must be an object created with arch_schema()", arg);
  }
...
static inline struct ArrowArray* array_data_from_xptr(SEXP array_data_xptr, const char* arg) {
  if (!Rf_inherits(array_data_xptr, "arch_array_data")) {
    Rf_error("`%s` must be an object created with arch_array_data()", arg);
  }
...
static inline struct ArrowArrayStream* array_stream_from_xptr(SEXP array_stream_xptr, const char* arg) {
  if (!Rf_inherits(array_stream_xptr, "arch_array_stream")) {
    Rf_error("`%s` must be an object created with arch_array_stream()", arg);
  }
