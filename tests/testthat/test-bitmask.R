...
test_that("as_arch_bitmask() works", {
  expect_identical(
  ...
  expect_identical(
    as_arch_bitmask(c(FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE)),
  ...
  expect_identical(as_arch_bitmask(logical()), new_arrow_bitmask())

  expect_identical(as_arch_bitmask("TRUE"), as_arch_bitmask(TRUE))
  b <- as_arch_bitmask(TRUE)
  expect_identical(as_arch_bitmask(b), b)
})
...
test_that("bitmask print method works", {
  ...
  expect_output(print(new_arrow_bitmask()), "arch_arrow_bitmask")
})
...
test_that("bitmask subset works", {
  ...
  expect_identical(new_arrow_bitmask(0xff)[1:2], as_arch_bitmask(c(TRUE, TRUE)))
})
